...
function rowify(columns) {
  ...
  for (var thisHdr in columns) {
    for (var thisPath in columns[thisHdr]) {
      if (found.hasOwnProperty(thisPath)) {
        break;
      }
      found[thisPath] = {};
...
var tabulate = {
  delimit: function(data, options) {
    ...
    var stringWrap =
      typeof options.stringWrap !== 'undefined' ? options.stringWrap : '"';
    var eol = typeof options.eol !== 'undefined' ? options.eol : '\n';
    var dateFormatter =
      typeof options.dateFormatter !== 'undefined'
    ...
    var escape =
      typeof options.escape === 'function'
        ? options.escape
        : function(string, wrap) {
            return string.replace(
              new RegExp(escapeRegExp(wrap), 'g'),
              (typeof options.escape === 'string' ? options.escape : wrap) +
                wrap
            );
          };
  ...
  array: function(data, options) {
    ...
    var includeHeaders =
      typeof options.includeHeaders !== 'undefined'
    ...
    if (sort) {
      rows.sort(function(a, b) {
        for (var i = 0; i < sort.length; i++) {
          ...
          if (index !== -1) {
            if (direction === 'asc' && a[index] < b[index]) {
              return -1;
            }
            if (direction === 'asc' && a[index] > b[index]) {
              return 1;
            }
            if (direction === 'desc' && a[index] > b[index]) {
              return -1;
            }
            if (direction === 'desc' && a[index] < b[index]) {
              return 1;
  ...
  html: function(data, options) {
    ...
    var array = this.array(data, options);
    var escapeInnerHTML =
      typeof options.escapeInnerHTML !== 'undefined'
        ? options.escapeInnerHTML
        : true;
    var convertLinks =
      typeof options.convertLinks !== 'undefined'
        ? options.convertLinks
        : false;
    ...
    result +=
      '<table' +
      (typeof classes.table !== 'undefined'
        ? ' class="' + htmlEntities(classes.table) + '"'
        : '') +
      '>' +
      eol;
    if (includeHeaders) {
      result +=
        separator +
        '<thead' +
        (typeof classes.thead !== 'undefined'
          ? ' class="' + htmlEntities(classes.thead) + '"'
          : '') +
        '>' +
        eol;
      result +=
        ...
        separator +
        '<tr' +
        (typeof classes.tr !== 'undefined'
          ? ' class="' + htmlEntities(classes.tr) + '"'
          : '') +
        '>' +
        eol;
      for (var i = 0; i < array[0].length; i++) {
        result +=
          ...
          separator +
          '<th' +
          (typeof classes.th !== 'undefined'
            ? ' class="' + htmlEntities(classes.th) + '"'
            : '') +
          '>';
    ...
    result +=
      separator +
      '<tbody' +
      (typeof classes.tbody !== 'undefined'
        ? ' class="' + htmlEntities(classes.tbody) + '"'
        : '') +
      '>' +
      eol;
    for (var row = 1; row < array.length; row++) {
      ...
      for (var col = 0; col < array[row].length; col++) {
        result +=
          ...
          separator +
          '<td' +
          (typeof classes.td !== 'undefined'
            ? ' class="' + htmlEntities(classes.td) + '"'
            : '') +
          '>';
        if (array[row][col] instanceof Date) {
          ...
          if (
            convertLinks &&
            /^(?:http|https):\/\/\S*$/.test(array[row][col])
          ) {
            result +=
              '<a href="' +
              htmlEntities(array[row][col]) +
              '">' +
              htmlEntities(array[row][col]) +
              '</a>';
          } else {
            result += !escapeInnerHTML
              ? array[row][col]
              : htmlEntities(array[row][col]);
          }
